...
describe('LargeList overflow semantics', () => {
    ...
    test(`.get() throws when an offset exceeds Number.MAX_SAFE_INTEGER`, () => {
        ...
        // Such offsets can only arise from malformed/out-of-bounds data: a well-formed batch's
        // offsets index into a child buffer that is physically present, bounded by what the
        // engine can allocate (far below 2^53). The guard fails loudly rather than truncating.
